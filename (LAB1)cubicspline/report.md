...
7. **Результат візуалізації:**
   Сформовані графіки були автоматично збережені за допомогою бібліотеки `matplotlib` у файл `goverla_comparison.png`.

![Порівняння точності сплайнів](goverla_comparison.png)  
...
На основі проведеного аналізу побудованих графіків на Рис. 1. можна зробити наступні **власні спостереження**:
* **10 вузлів (зелений пунктир):** Модель досить груба і згладжує багато важливих перепадів висоти маршруту. Також спостерігається просадка кривої сплайну нижче реальної висоти між деякими пікетами через великий крок сітки та малу кількість точок для інтерполяції.
* **15 вузлів (пурпурна лінія):** Значно краще наближення, яке повторює більшість вигинів оригінального маршруту. Втім, на ділянках із різкими змінами рельєфу (ближче до самої вершини), значення все ще мають відхилення від ідеалу.
* **20 вузлів (синя лінія):** Сплайн майже ідеальним чином описує реальні дані. Густа сітка повністю нівелює осциляції або перезгладжування, забезпечуючи високоточний та фізично реалістичний профіль висоти підйому.
